...
import { DISTRICT_NAME_VARIANTS, DISTRICT_DEMOGRAPHICS, ALLOWED_DISTRICTS, GENDER, CENSUS_PROJECTION_DATA, CENSUS_URBAN_RURAL_DATA, CENSUS_STATS_DATA } from '../../data/comparativeData';
...
const InfoTooltip = ({ text, position = 'top' }: { text: string; position?: 'top' | 'bottom'; source?: string }) => (
    ...
    <span className="group/info inline-block ml-2 align-middle z-[100]">
        ...
        <span className={`absolute left-0 right-0 px-1 hidden group-hover/info:block animate-in fade-in zoom-in-95 duration-200 pointer-events-none z-[110] 
...
export const MapSection: React.FC<MapSectionProps> = ({
    ...
    const tooltipSource = isCensusSource ? 'Census Stat. Projection' : 'UNFPA AI/ML Model v1';
    ...
    const ageDistributionData = React.useMemo(() => {
        ...
        const getSum = (key: string) => {
            if (!isAllDistricts && districtData) {
                ...
                return allDistrictsData.reduce((acc, d) => acc + (parseFloat(d[key] || 0) || 0), 0);
            }
            return 0;
        };

        const ageGroupsList = [
            { label: '0-9', m: [`male_${year}_0_12`, `male_${year}_1_4`, `male_${year}_5_9`], f: [`female_${year}_0_12`, `female_${year}_1_4`, `female_${year}_5_9`] },
            { label: '10-19', m: [`male_${year}_10_14`, `male_${year}_15_19`], f: [`female_${year}_10_14`, `female_${year}_15_19`] },
            { label: '20-29', m: [`male_${year}_20_24`, `male_${year}_25_29`], f: [`female_${year}_20_24`, `female_${year}_25_29`] },
            { label: '30-39', m: [`male_${year}_30_34`, `male_${year}_35_39`], f: [`female_${year}_30_34`, `female_${year}_35_39`] },
            { label: '40-49', m: [`male_${year}_40_44`, `male_${year}_45_49`], f: [`female_${year}_40_44`, `female_${year}_45_49`] },
            { label: '50-59', m: [`male_${year}_50_54`, `male_${year}_55_59`], f: [`female_${year}_50_54`, `female_${year}_55_59`] },
            { label: '60-69', m: [`male_${year}_60_64`, `male_${year}_65_69`], f: [`female_${year}_60_64`, `female_${year}_65_69`] },
            { label: '70-79', m: [`male_${year}_70_74`, `male_${year}_75_79`], f: [`female_${year}_70_74`, `female_${year}_75_79`] },
            { label: '80-89', m: [`male_${year}_80_84`, `male_${year}_85_89`], f: [`female_${year}_80_84`, `female_${year}_85_89`] },
    ...
    }, [selectedYear, selectedDistrictName, districtData, allDistrictsData, districtsLookup]);
    ...
    const CustomAgeTooltip = ({ active, payload, label }: any) => {
        if (active && payload && payload.length) {
            const m = payload.find((p: any) => p.dataKey === 'male')?.payload.maleAbs || 0;
            ...
            return (
                <div className="bg-white border border-gray-200 p-2 rounded-lg shadow-sm text-xs text-left">
                    ...
                    <p className="text-[#0868ac] font-mono"><span className="font-bold text-gray-600">Male:</span> {formatter.format(m)}</p>
                    <p className="text-[#F96000] font-mono"><span className="font-bold text-gray-600">Female:</span> {formatter.format(f)}</p>
                </div>
            );
        }
        return null;
    ...
    useEffect(() => {
        if (districtData) {
            const rawName = districtData.district_name || districtData.NAME || districtData.name || 'Selected Area';
            const name = DISTRICT_NAME_VARIANTS[rawName] || rawName;
    ...
    useEffect(() => {
        if (targetDistrict) {
            const mappedName = targetDistrict === 'Odisha' ? 'All Districts' : targetDistrict;
    ...
    useEffect(() => {
        const handleScroll = () => {
            if (window.scrollY > 10 || window.pageYOffset > 10 || document.documentElement.scrollTop > 10) {
    ...
    }, [activeLayer, selectedYear, selectedGender, selectedRegion, selectedDistrictName]);
    ...
    */
    ...
    const getPopForYear = (districtName: string, year: string, gender: 'sum' | 'Male' | 'Female' = 'sum', forceModel: boolean = false) => {
        ...
        if (isCensusSource && !forceModel) {
            ...
            if (censusVal !== undefined) {
                if (gender === 'Male') return censusVal * 0.50;
                if (gender === 'Female') return censusVal * 0.50;
                return censusVal;
            }
        }
    ...
    const getStats = () => {
        const dName = (selectedDistrictName && selectedDistrictName !== 'All Districts') 
            ? selectedDistrictName 
            : (districtData ? (districtData.district_name || districtData.NAME || districtData.name || 'Selected Area') : 'All Districts');
        ...
        const getPopDataForYear = (year: number) => {
            ...
            const avYears = Object.keys(demo).map(Number).sort((a, b) => a - b);
            if (avYears.length === 0) return null;
            const closest = avYears.reduce((prev, curr) => Math.abs(curr - year) < Math.abs(prev - year) ? curr : prev);
            return demo[closest];
        };

        const currentDemo = getPopDataForYear(yearInt);
        const totalP = getPopForYear(name, yearSuffix, 'sum') as number || (currentDemo ? currentDemo.male + currentDemo.female : 41974218);
        const mP = getPopForYear(name, yearSuffix, 'Male') as number || (currentDemo ? currentDemo.male : 21196980);
        const fP = getPopForYear(name, yearSuffix, 'Female') as number || (currentDemo ? currentDemo.female : 20777238);

        if (isCensusSource) {
            // Use Model data for gender metrics rendering on Census tab
            const latestModelPop = getPopForYear(name, yearSuffix, 'sum', true) as number || (currentDemo ? currentDemo.male + currentDemo.female : 41974218);
            let modelMaleCount = getPopForYear(name, yearSuffix, 'Male', true) as number;
            let modelFemaleCount = getPopForYear(name, yearSuffix, 'Female', true) as number;

            // Fallback to static demographics if missing model data
            if (!modelMaleCount && !modelFemaleCount && DISTRICT_DEMOGRAPHICS[name] && DISTRICT_DEMOGRAPHICS[name][yearInt]) {
            ...
            const modelMalePct = ((modelMaleCount / modelTotalForGender) * 100).toFixed(1) + '%';
            const modelFemalePct = ((modelFemaleCount / modelTotalForGender) * 100).toFixed(1) + '%';

            const area = districtData?.['Shape_Area'] || districtData?.['AREA'] || districtData?.['Area'] || districtData?.['area'] || (nameForLookup === 'Odisha' ? 155707 : 5000);
            const censusStat = CENSUS_STATS_DATA[nameForLookup]?.[appliedFilters.year];
            ...
            return {
                ...
                area: area,
                density: censusDensity !== undefined 
                    ? (censusDensity % 1 === 0 ? censusDensity.toString() : censusDensity.toFixed(2)) 
                    : '350',
                literacy: '72.9%',
        ...
        if (!maleCount && !femaleCount && DISTRICT_DEMOGRAPHICS[name] && DISTRICT_DEMOGRAPHICS[name][yearInt]) {
        ...
        if (densityValue === '—' && DEMOGRAPHIC_STATS[name === 'All Districts' ? 'Odisha' : name]?.[yearInt]) {
            densityValue = DEMOGRAPHIC_STATS[name === 'All Districts' ? 'Odisha' : name][yearInt].density;
        }
        ...
        if (match) {
            return (
                <>
                    ...
                    <span className={`font-semibold ${unitSize} text-gray-400`}> {match[2]}</span>
                </>
            );
        }
        return val;
                    ...
                    <div className="w-full h-[450px] lg:flex-1 relative overflow-hidden lg:h-[80vh] bg-white rounded-lg shadow-sm border border-gray-100 group">
                        <MapComponent
                            ...
                            onResetClick={() => {
                                ...
                                setAppliedFilters(prev => ({ ...prev, district: 'All Districts' }));
                            }}
                            onDistrictClick={(data) => {
                                const rawName = data.district_name || data.NAME || data.name || 'Selected Area';
                                const name = DISTRICT_NAME_VARIANTS[rawName] || rawName;
                        ...
                        `}} />
                            ...
                            <div className="p-5 pb-4 sticky top-0 bg-white/90 backdrop-blur-md z-[70] shrink-0 border-b border-gray-200">
                                <h3 className="text-[16px] font-bold text-gray-900 leading-tightt mb-2 uppercase">Filter / Selection</h3>
                                <p className="text-[10px] text-gray-500 leading-relaxed">
                                    Configure viewing preferences. Apply geographical filters, temporal ranges, and demographic slices.
                                </p>
                            </div>
                            ...
                            <div className="p-5 flex-1 overflow-y-auto custom-scrollbar flex flex-col gap-5">
                                ...
                                <div className="relative w-full">
                                    <span className="text-[10px] font-black uppercase tracking-widest mb-2 opacity-70 block">District</span>
                                    <div className="flex items-center gap-2">
                                        {(() => {
                                            const sortedDistricts = [...ALLOWED_DISTRICTS].sort();
                                            const selectedIndex = sortedDistricts.indexOf(selectedDistrictName);

                                            let theme = { bg: 'bg-white', text: 'text-gray-600', border: 'border-gray-400' };

                                            if (selectedDistrictName !== 'All Districts' && selectedIndex !== -1) {
                                                const flavorIndex = selectedIndex % 6;
                                                const flavors = [
                                                    { bg: 'bg-[#bae4bc]', text: 'text-gray-600', border: 'border-gray-400' },
                                                    { bg: 'bg-[#bae4bc]', text: 'text-gray-600', border: 'border-gray-400' },
                                                    { bg: 'bg-[#bae4bc]', text: 'text-gray-600', border: 'border-gray-400' },
                                                    { bg: 'bg-[#bae4bc]', text: 'text-gray-600', border: 'border-gray-400' },
                                                    { bg: 'bg-[#bae4bc]', text: 'text-gray-600', border: 'border-gray-400' },
                                            ...
                                            return (
                                                <div className={`flex-1 px-3 py-1.5 text-[12px] tracking-wide bg-white text-gray-700 font-semibold border rounded-md ${theme.border} truncate h-full flex items-center transition-colors`}>
                                                    {selectedDistrictName === 'All Districts' ? "All" : selectedDistrictName}
                                                </div>
                                            );
                                        })()}
                                        <button
                                        ...
                                        >
                                            <ChevronDown className={`w-5 h-5 text-[#f64e24] transition-transform ${openDropdown === 'district' ? 'rotate-180' : ''}`} />
                                        </button>
                                    </div>
                                    {openDropdown === 'district' && (
                                        <div className="absolute left-0 top-full mt-2 max-h-[300px] overflow-y-auto custom-scrollbar bg-white/95 backdrop-blur-md rounded-lg shadow-xl border border-gray-100 p-2 z-[50] animate-in fade-in slide-in-from-top-2">
                                            ...
                                            {ALLOWED_DISTRICTS.slice().sort().map((name, index) => {
                                                const flavors = [
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className="w-full px-3 py-2 text-left transition-all hover:bg-gray-50/50"
                                                    >
                                                        <span className={`text-xs tracking-wider border-b-2 pb-0.5 font-semibold ${theme.text} ${theme.border}`}>
                                                            {name}
                                                        </span>
                                                    </button>
                                                )
                                ...
                                <div className="relative w-full">
                                    <span className="text-[10px] font-black uppercase tracking-widest mb-2 opacity-70 block">Layer</span>
                                    <div className="flex items-center gap-2">
                                        {(() => {
                                            return (
                                                <div className={`flex-1 px-3 py-1.5 text-[12px] tracking-wide bg-white text-gray-700 font-semibold rounded-md border border-gray-400 truncate h-full flex items-center transition-colors min-w-[140px]`}>
                                                    {layers.find(l => l.id === activeLayer)?.label}
                                                </div>
                                            );
                                        })()}
                                        <button
                                            onClick={() => setOpenDropdown(openDropdown === 'layer' ? null : 'layer')}
                                            className="h-full px-1 hover:bg-gray-100 transition-colors flex items-center justify-center border border-transparent hover:border-gray-200"
                                        >
                                            <ChevronDown className={`w-5 h-5 text-[#f64e24] transition-transform ${openDropdown === 'layer' ? 'rotate-180' : ''}`} />
                                        </button>
                                    </div>
                                        ...
                                        <div className="absolute left-0 top-full mt-2 bg-white/95 backdrop-blur-md rounded-lg shadow-xl border border-gray-100 p-2 z-[50] animate-in fade-in slide-in-from-top-2">
                                            {layers.map((l, index) => {
                                                ...
                                                return (
                                                    <button
                                                        key={l.id}
                                                        onClick={() => { setActiveLayer(l.id as LayerType); setOpenDropdown(null); }}
                                                        className="w-full px-3 py-2 text-left transition-all hover:bg-gray-50/50"
                                                    >
                                                        <span className={`text-xs tracking-wider border-b-2 pb-0.5 font-semibold ${theme.text} ${theme.border}`}>
                                                            {l.label}
                                ...
                                <div className="flex items-center justify-between px-1">
                                    <span className="text-[10px] font-black uppercase tracking-widest opacity-70">Show Subdistrict</span>
                                    <button
                                ...
                                <div className="w-full">
                                    <div className="flex justify-between items-center mb-2">
                                        <span className="text-[10px] font-black uppercase tracking-widest opacity-70">Year</span>
                                        <span className="text-sm font-black text-[#0868ac] font-mono">{selectedYear}</span>
                                    </div>
                                    <input
                                        type="range"
                                        min={layers.find(l => l.id === activeLayer)?.minYear || 2011}
                                        max={layers.find(l => l.id === activeLayer)?.maxYear || 2035}
                                        step="2"
                                        value={selectedYear}
                                    ...
                                    <div className="flex justify-between mt-1">
                                        <span className="text-[9px] font-bold text-gray-400">{layers.find(l => l.id === activeLayer)?.minYear || 2011}</span>
                                        <span className="text-[9px] font-bold text-gray-400">{layers.find(l => l.id === activeLayer)?.maxYear || 2035}</span>
                                    </div>
                                </div>
                            ...
                            {(() => {
                                const steps = LAYER_SCALES[activeLayer ?? ''] ?? [0, 25, 50, 75, 100];
                                        ...
                                        <div className="flex flex-col gap-1.5 font-semibold">
                                            {activeLayer === 'deg_urbanisation' ? (
                                                ...
                                                <div key={id} className="flex items-center gap-2.5">
                                                    <div className="w-6 h-3 rounded-full" style={{ backgroundColor: item.color }}></div>
                                                    <span className="text-[11px] text-gray-800 font-medium tracking-wide">{item.label}</span>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                    ...
                    <div className="w-full lg:w-80 h-auto lg:h-[80vh] bg-white border border-gray-100 rounded-lg flex flex-col z-20 shadow-sm transition-all hover:shadow-md relative">
                                ...
                                <div className="flex-1">
                                    ...
                                    {stats.name === 'All Districts' && (
                                        <p className="mt-1 font-bold">
                                            <span >
                                                <span className="text-xs font-semibold uppercase text-gray-600">Area :</span>   <span className="text-xs font-mono text-[#0868ac] tracking-wider">{stats.area ? stats.area.toLocaleString() : 'N/A'}</span>
                                            </span> <span className='text-xs text-gray-600'>km²</span>
                                        </p>
                                    )}
                        ...
                        <div className="flex-1 overflow-y-auto p-6 pt-4 custom-scrollbar space-y-8">
                            <div className="flex items-center justify-between">
                                <span className='text-[14px] text-gray-600 font-bold uppercase tracking-wider'>Key Stats</span>
                            ...
                            <div className="text-[10px] text-gray-500 font-medium text-right w-full mt-[-22px]">
                                {isCensusSource ? 'Based on official census data and projections.' : 'Generated using AI/ML models and satellite data.'}
                            </div>
                            ...
                            <div className="grid mt-4">
                                {(() => {
                                    ...
                                    const districtNameForStats = stats.name === 'All Districts' ? 'Odisha' : stats.name;

                                    const popCurr = getPopForYear(districtNameForStats, selectedYear.toString());
                                    const popPrev = getPopForYear(districtNameForStats, prevYear.toString());

                                    let growth = 0;
                                    if (isCensusSource) {
                                        const censusStat = CENSUS_STATS_DATA[districtNameForStats]?.[selectedYear.toString()];
                                        growth = (censusStat?.growth !== null && censusStat?.growth !== undefined) ? censusStat.growth : 1.25;
                                        ...
                                        const statRecord = DEMOGRAPHIC_STATS[districtNameForStats]?.[selectedYear];
                                        ...
                                        <div className="flex w-full bg-white p-4 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-all group/stat items-center justify-between gap-2 relative">
                                            ...
                                            <div className="w-3/4">
                                                <div className="flex flex-wrap items-baseline gap-2 gap-y-1 mb-1">
                                                    <p className="text-2xl font-black text-gray-900 tracking-tight">{formatStatValue(stats.pop)}</p>
                                                    <div className={`flex items-center text-[10px] font-bold ${isPositive ? 'text-green-600 bg-green-50' : 'text-red-600 bg-red-50'} px-1.5 py-0.5 rounded-md`}>
                                                        {isPositive ? <ArrowUpRight className="w-3 h-3 mr-1" strokeWidth={3} /> : <ArrowDownRight className="w-3 h-3 mr-1" strokeWidth={3} />}
                                                        {Math.abs(growth).toFixed(2)}%
                                                    </div>
                                                </div>
                                                <p className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">
                                                    Total Population
                                                    <InfoTooltip text="Projected total population count." position="bottom" source={tooltipSource} />
                                                </p>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div className="bg-white p-4 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-all relative">
                                    <div className="flex flex-col">
                                        ...
                                        <span className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">
                                            Pop. Density
                                            <InfoTooltip text="Average number of people per square kilometer of land area." source={tooltipSource} />
                                        </span>
                                    </div>
                                </div>
                                ...
                                <div className="bg-white p-4 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-all relative">
                                    <div className="flex flex-col">
                                            ...
                                            <div className="items-baseline gap-x-2 gap-y-1 mb-1">
                                                <span className="text-xl font-black text-[#0868ac] tracking-tight leading-none group-hover/stat:scale-105 transition-transform origin-left font-mono">
                                                    {(() => {
                                                        ...
                                                        const dName = stats.name === 'All Districts' ? 'Odisha' : stats.name;

                                                        if (isCensusSource) {
                                                            const censusStat = CENSUS_STATS_DATA[dName]?.[currentYear.toString()];
                                                            if (censusStat?.growth !== null && censusStat?.growth !== undefined) {
                                                                return censusStat.growth.toFixed(2);
                                                            }
                                                        }

                                                        const popCurr = getPopForYear(dName, currentYear.toString());
                                                        const popPrev = getPopForYear(dName, prevYear.toString());

                                                        if (popCurr && popPrev && popPrev !== 0) {
                                                            const growth = ((popCurr - popPrev) / popPrev) * 100;
                                                            return growth.toFixed(2);
                                                        }

                                                        // Fallback: Check if growth is explicitly provided in data
                                                        const pmtilesGrowthKey = `growth_${currentYear}`;
                                                        let val = districtData?.[pmtilesGrowthKey] || districtsLookup.get(dName)?.[pmtilesGrowthKey];

                                                        if (val === undefined || val === null) {
                                                            val = DEMOGRAPHIC_STATS[dName]?.[currentYear]?.growth;
                                                        }
                                        ...
                                        <span className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">
                                            Growth <span className="text-[9px] text-gray-400 lowercase ml-1 font-medium italic"> (vs. {parseInt(appliedFilters.year) - 1})</span>
                                            <InfoTooltip text="Annual percentage change in population size compared to the previous year." source={tooltipSource} />
                                        </span>
                                    </div>
                                </div>
                            ...
                            {(() => {
                                ...
                                const demographicData = DEMOGRAPHIC_STATS[currentName]?.[parseInt(appliedFilters.year)];
                                if (!isCensusSource && (!demographicData || !demographicData.urban)) return null;

                                const urbanData = isCensusSource ? CENSUS_URBAN_RURAL_DATA[currentName]?.[appliedFilters.year] : null;
                                const urban = isCensusSource
                                    ? (urbanData?.urban || 0.6 * (getPopForYear(currentName, appliedFilters.year) as number))
                                    : demographicData.urban;
                                const rural = isCensusSource
                                    ? (urbanData?.rural || 0.4 * (getPopForYear(currentName, appliedFilters.year) as number))
                                ...
                                return (
                                    <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-all relative">
                                        <div className="flex items-center gap-1 text-[10px] font-medium text-gray-700 mb-2">
                                            <span className="font-bold uppercase tracking-wider text-gray-500">Urban /</span> <span className="font-bold  tracking-wider uppercase text-gray-500">Rural</span>
                                            <InfoTooltip text="Distribution of people living in urban centers versus rural localities." source={tooltipSource} />
                                        </div>
                                        <div className="flex justify-between items-center mb-3">
                                            <div className="font-bold">
                                                <span className="text-[#0868ac] text-xl font-mono">{formatLakhs(urban)}</span> <span className='text-[10px] text-gray-500'> L </span> <span className="text-gray-400"> / </span> <span className="text-xl text-[#0868ac] font-mono">{formatLakhs(rural)}</span> <span className='text-[10px] text-gray-500'> L</span>
                                            </div>
                                        </div>
                                        <div className="w-full h-3 rounded-full flex overflow-hidden">
                                            <div className="bg-gray-400 h-full transition-all duration-500" style={{ width: `${urbanPercent}%` }}></div>
                                            <div className="bg-[#0868ac] h-full transition-all duration-500" style={{ width: `${ruralPercent}%` }}></div>
                                        </div>
                                    </div>
                                );
                            })()}
                            ...
                            {isCensusSource && (
                                <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-all relative">
                                    <div className="flex items-center gap-1 text-[10px] font-medium text-gray-700 mb-4">
                                        <span className="font-bold uppercase tracking-wider text-gray-500">Gender Distribution</span>
                                        <InfoTooltip text="Breakdown of the population by male and female counts." source={tooltipSource} />
                                    </div>
                                    ...
                                    <div className="flex items-center justify-between gap-4">
                                        ...
                                        <div className="space-y-4">
                                            <div>
                                                <p className="!text-[14px] font-bold text-gray-500 mb-0.5">{formatStatValue(stats.maleCount, 'text-[14px]')}</p>
                                                <p className="text-xl font-black text-gray-900 tracking-tight leading-none">{formatStatValue(stats.male, 'text-[10px]')}</p>
                                                <p className="text-[10px] uppercase font-bold text-gray-500 mt-1 tracking-wider">
                                                    Male
                                                </p>
                                            </div>
                                            <div>
                                                <p className="!text-[14px] font-bold text-gray-500 mb-0.5">{formatStatValue(stats.femaleCount, 'text-[14px]')}</p>
                                                <p className="text-xl font-black text-gray-900 tracking-tight leading-none">{formatStatValue(stats.female, 'text-[10px]')}</p>
                                                <p className="text-[10px] uppercase font-bold text-gray-500 mt-1 tracking-wider">
                                                    Female
                                                </p>
                                            </div>
                                        ...
                                        <div className="w-32 h-32 relative">
                                            <ResponsiveContainer width="100%" height="100%">
                                                <PieChart>
                                                    ...
                                                    <Pie
                                                        data={[
                                                            { name: 'Male', value: parseFloat(stats.male) || 50 },
                                                            { name: 'Female', value: parseFloat(stats.female) || 50 }
                                                        ]}
                                                        cx="50%"
                                                        cy="50%"
                                                        innerRadius={30}
                            ...
                            {isCensusSource && selectedDistrictName !== 'All Districts' && selectedDistrictName !== 'Odisha' && (
                                <div className="bg-white p-5 rounded-lg border border-gray-100 shadow-sm hover:shadow-md transition-all relative">
                                    <div className="flex items-center gap-1 text-[10px] font-medium text-gray-700 mb-4">
                                        <span className="font-bold uppercase tracking-wider text-gray-500">Age Distribution</span>
                                        <InfoTooltip text="Age composition of the population, showing male and female distribution across birth cohorts." source={tooltipSource} />
                                    </div>
                                    <div className="w-full h-70">
                                        {(() => {
                                            const data = ageDistributionData;
                                            const maxVal = Math.max(1, ...data.map(d => Math.max(Math.abs(d.male), d.female)));
                                                ...
                                                <ResponsiveContainer width="100%" height="100%">
                                                    ...
                                                    >
                                                        ...
                                                        <YAxis
                                                            ...
                                                            width={30}
                                                        />
                                                        <Tooltip content={<CustomAgeTooltip />} cursor={{ fill: '#f3f4f6' }} />
                                                        <Legend
                                                            iconType="circle"
                                                            wrapperStyle={{ fontSize: '10px', paddingTop: '10px' }}
                                                            formatter={(value) => <span style={{ color: '#6b7280', fontWeight: 'bold' }}>{value.toUpperCase()}</span>}
                                                        />
                                                        <Bar
                                                            dataKey="male"
                                                            fill="#0868ac"
