...
import { Layers, Map as MapIcon, Plus, Minus, Home, ChevronDown } from 'lucide-react';
import { LULC_STATS, DISTRICT_NAME_VARIANTS, ALLOWED_DISTRICTS } from '../../data/comparativeData';
...
const CATEGORIES = [
    'Water', 'Trees', 'Flooded Vegetation', 'Crops', 'Built Area', 'Bare Ground', 'Grass', 'Rangeland'
];

const MapLulc: React.FC<MapLulcProps> = ({ selectedDistrict = 'Odisha', onDistrictSelect }) => {
    ...
    useEffect(() => {
            ...
            if (!m.getLayer('district-outline')) {
                ...
                m.on('mousemove', 'subdistrict-fill', (e) => {
                    ...
                    if (feature) {
                        ...
                        const subName = props?.subdistrict_name || props?.SUBDIST_NAM || 'Unknown Area';
                ...
                m.on('click', 'subdistrict-fill', (e) => {
                    if (e.features?.[0]) {
                        const feature = e.features[0];
                        const rawName = feature.properties?.district_name || feature.properties?.DIST_NAME || feature.properties?.NAME || feature.properties?.name;
                        if (rawName) {
        ...
        m.on('sourcedata', (e) => {
            if (e.sourceId === 'district-source' && e.isSourceLoaded) {
                const features = m.querySourceFeatures('district-source', { sourceLayer: 'zcta' });
                let hasChanges = false;
                features.forEach((f: any) => {
                    const rawName = f.properties?.district_name || f.properties?.NAME || f.properties?.name;
                    if (rawName) {
                        const canonicalName = (DISTRICT_NAME_VARIANTS[rawName] || rawName).toLowerCase();
                        ...
                        const coords = (f.geometry as any).coordinates?.toString().substring(0, 100);
                        if (!existing.some((ef: any) => (ef.geometry as any).coordinates?.toString().substring(0, 100) === coords)) {
                            existing.push(f);
                            hasChanges = true;
                        }
                    }
    ...
    useEffect(() => {
        ...
        const isOdisha = !selectedDistrict || selectedDistrict === 'Odisha' || selectedDistrict === 'All Districts';

        if (m.getLayer('district-outline')) {
            m.setPaintProperty('district-outline', 'line-color', isOdisha ? '#000' : [
                'case',
                ['any',
                    ['==', ['get', 'district_name'], selectedDistrict],
                    ['==', ['get', 'NAME'], selectedDistrict]
            ...
            m.setPaintProperty('district-outline', 'line-width', isOdisha ? 1 : [
                'case',
                ['any',
                    ['==', ['get', 'district_name'], selectedDistrict],
                    ['==', ['get', 'NAME'], selectedDistrict]
            ...
            m.setPaintProperty('district-outline', 'line-opacity', isOdisha ? 0.2 : 0.8);
        }
        ...
        if (m.getLayer('district-mask')) {
            ...
            m.setPaintProperty('district-mask', 'fill-opacity', isOdisha ? 0 : [
                'case',
                ['any',
                    ['==', ['get', 'district_name'], selectedDistrict],
                    ['==', ['get', 'NAME'], selectedDistrict]
        ...
        let lookupKey = selectedDistrict.toLowerCase();
        let features: any[] = [];
    ...
    useEffect(() => {
        ...
        const colors = [
            "#419BDF", "#397D49", "#000000", "#7A87C6", "#E49635",
            "#000000", "#C4281B", "#A59B8F", "#000000", "#000000", "#F0CF0E"
        ];
        const params = `#color:["${colors.join('","')}"],1,11,c`;
        const yearUrl = `cog://https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/lulc/odisha_lulc_${selectedYearLulc}.tif${params}`;
                            ...
                            <>
                                ...
                                <div className="absolute right-0 top-full mt-3 bg-white rounded-md shadow-xl border border-gray-100 p-3 z-[200] animate-in fade-in slide-in-from-top-2 min-w-[120px] transition-all">
                                    {[...years].reverse().map((y) => (
                                        ...
                                        >
                                            <span className={`text-xs font-bold tracking-wider border-b-2 pb-0.5 transition-all ${selectedYearLulc === y.toString() ? 'text-[#F96000] border-orange-400' : 'text-gray-600 border-gray-400'}`}>
                                                {y}
                                            </span>
                                        </button>
                                    ))}
            ...
            <div className="flex flex-col lg:flex-row gap-4">
                ...
                <div className="w-full lg:w-[40%] h-[500px] bg-gray-100 rounded-lg overflow-hidden border border-gray-200 relative">
                    ...
                    <div className="absolute bottom-4 left-4 z-20 bg-white/90 backdrop-blur-md p-3 rounded-xl shadow-lg border border-gray-100 max-w-[180px]">
                        <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">LULC</div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-gray-50 border-b border-gray-200 sticky top-0 z-10">
                                        ...
                                        {years.map(y => (
                                            <th key={y} className="p-2 text-[10px] font-bold text-gray-500 uppercase text-center border-l border-gray-100">{y} (sq.km)</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
                                    {CATEGORIES.map(cat => (
                                        <tr key={cat} className="border-b border-gray-100 hover:bg-gray-50/50 transition-colors">
                                            <td className="p-2 text-[10px] font-bold text-gray-700">{cat}</td>
                                            {years.map(y => {
                                                ...
                                                return (
                                                    <td key={y} className="p-2 text-center border-l border-gray-50">
                                                        <div className="text-[10px] font-bold text-gray-900">{val.toFixed(1)}</div>
                                                        {growth !== null && (
                                                            <div className={`text-[8px] font-black ${growth >= 0 ? 'text-green-600' : 'text-red-500'}`}>
