...
import { Trophy, AlertCircle, Map as MapIcon, ChartNoAxesColumnIncreasing, X, ChevronLeft, ChevronRight, ChevronDown, Check } from 'lucide-react';
import {
    XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, LineChart, Line, AreaChart, Area, ReferenceArea, LabelList
} from 'recharts';
...
export const StatsDetails: React.FC<StatsDetailsProps> = ({ selectedDistrict = 'Odisha', onDistrictSelect, data, allDistrictsData }) => {
    const [year1, setYear1] = useState(2011);
    const [year2, setYear2] = useState(2024);
    const [compareLayer, setCompareLayer] = useState<LayerType | 'builtup' | 'lulc'>('builtup' as any);
    ...
    const [chartDistricts, setChartDistricts] = useState<string[]>(DISTRICT_NAMES.slice(0, 5));
    const [openDistrictSelector, setOpenDistrictSelector] = useState(false);
    const [projectionMode, setProjectionMode] = useState<'Model Only' | 'Model Vs Census Projection'>('Model Only');
    ...
    React.useEffect(() => {
        // Change Analysis cleanup moved to ChangeAnalysis component
    }, [selectedDistrict]);
    ...
    // Sync map bounds when data changes (from parent MapComponent)
    // Sync map bounds and show sentinel when a district is selected
    React.useEffect(() => {
        if (selectedDistrict && selectedDistrict !== 'Odisha' && selectedDistrict !== 'All Districts') {
            if (data && data.bounds) {
    ...
    const getPopForYear = (districtName: string, year: string, gender: 'sum' | 'Male' | 'Female' = 'sum') => {
        const propKey = gender === 'sum' ? `pop_${year}_sum` : (gender === 'Male' ? `male_${year}` : `female_${year}`);
        ...
        if (selectedDistrict === districtName && data) {
            ...
            if (val !== undefined && val !== null) return typeof val === 'string' ? parseFloat(val) : val;
        }
    ...
    const availableTrendYears = React.useMemo(() => {
        ...
        if (allDistrictsData && allDistrictsData.length > 0) {
            ...
            const dataYears = Object.keys(first)
                .filter(k => /^pop_\d{4}_sum$/.test(k)) // Matches pop_YYYY_sum
                .map(k => k.split('_')[1])
                .sort();
    ...
    // Sync chart districts with selected district from map
    React.useEffect(() => {
        if (selectedDistrict && selectedDistrict !== 'Odisha' && selectedDistrict !== 'All Districts') {
    ...
    const isDistrictSelected = selectedDistrict && selectedDistrict !== 'Odisha' && selectedDistrict !== 'All Districts';

    // Reset projection mode to Model Only if no district is selected
    React.useEffect(() => {
        if (!isDistrictSelected && projectionMode === 'Model Vs Census Projection') {
    ...
    const DISTRICT_PERFORMANCE = React.useMemo(() => {
        const rawData = DISTRICT_NAMES.map((name) => {
            ...
            const yearForGrowth = '2023';

            const pmtilesPop = getPopForYear(name, yearForTable) as number || 0;
            const pmtilesPop2023 = getPopForYear(name, yearForGrowth) as number || 0;
            ...
            if (DEMOGRAPHIC_STATS[name] && DEMOGRAPHIC_STATS[name][2025]) {
                ...
                const totalGrowth = ((pmtilesPop - pmtilesPop2023) / pmtilesPop2023) * 100;
                const yearlyGrowth = totalGrowth / 2;
                growthStr = (yearlyGrowth >= 0 ? '+' : '') + yearlyGrowth.toFixed(1) + '%';
            }

            let latestPop = pmtilesPop;
            let malePop = 0;
            let femalePop = 0;
            ...
            } else if (DISTRICT_DEMOGRAPHICS[name] && DISTRICT_DEMOGRAPHICS[name][yearInt]) {
            ...
            return {
                ...
                density: latestDensityValue > 0 ? latestDensityValue.toLocaleString(undefined, { minimumFractionDigits: 1, maximumFractionDigits: 1 }) + '/km²' : (400 + (seed * 50)).toFixed(0) + '/km²',
    ...
    }, [availableTrendYears, allDistrictsData, data, selectedDistrict]); // Recalculate when data changes

    // Calculate visible districts for the table
    // const selectedDistrictIndex = DISTRICT_PERFORMANCE.findIndex(d => d.name === selectedDistrict);
    let visibleDistricts = DISTRICT_PERFORMANCE;
        ...
        setTimeout(() => {
            ...
            if (selectedDistrict && selectedDistrict !== 'Odisha') {
                ...
                if (el) {
                    ...
                    const topOffset = el.offsetTop - (container.clientHeight / 2) + (el.clientHeight / 2);
    ...
    const getProjectionData = () => {
        ...
        return years.map(y => {
            ...
            chartDistricts.forEach(distName => {
                ...
                if (modelVal === null && MODEL_PROJECTION_DATA[distName] && MODEL_PROJECTION_DATA[distName][y]) {
                ...
                if (projectionMode === 'Model Vs Census Projection') {
                    if (CENSUS_PROJECTION_DATA[distName] && CENSUS_PROJECTION_DATA[distName][y]) {
    ...
    const displayDistricts = isModelVsCensus ? [selectedDistrict] : chartDistricts;
    ...
    return (
        <div className="bg-white min-h-[500px]">
            <div className="w-full mx-auto px-4 lg:px-6 py-10 space-y-12">
                ...
                <div className="space-y-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="relative group">
                                <div className="flex bg-gray-100 p-1 rounded-lg border border-gray-200">
                                    ...
                                    <button
                                        ...
                                        onClick={() => setProjectionMode('Model Vs Census Projection')}
                            ...
                            <div className={`relative flex items-center gap-2 group transition-all ${isModelVsCensus ? 'opacity-60 pointer-events-none' : ''}`}>
                                <div
                                    onClick={() => !isModelVsCensus && setOpenDistrictSelector(!openDistrictSelector)}
                                    className="flex flex-wrap items-center gap-1.5 p-1 px-2 min-h-[38px] bg-white border rounded-md min-w-[200px] cursor-pointer hover:bg-gray-50 transition-colors"
                                >
                                    {(isModelVsCensus ? [selectedDistrict] : chartDistricts).map(district => (
                                        <div
                                            key={district}
                                        ...
                                        >
                                            ...
                                            {!isModelVsCensus && (
                                                <button
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        setChartDistricts(prev => prev.filter(d => d !== district));
                                                    }}
                                    ...
                                    {!isModelVsCensus && chartDistricts.length === 0 && (
                                        <span className="px-3 text-[12px] font-black text-gray-400">Select Districts</span>
                                    )}
                                </div>
                                <button
                                    onClick={() => !isModelVsCensus && setOpenDistrictSelector(!openDistrictSelector)}
                                ...
                                >
                                    <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${openDistrictSelector ? 'rotate-180' : ''}`} strokeWidth={2.5} />
                                </button>
                                    ...
                                    <>
                                        ...
                                        <div className="absolute right-0 top-full mt-2 w-64 bg-white rounded-xl shadow-2xl border border-gray-100 z-[200] max-h-[400px] overflow-hidden flex flex-col animate-in fade-in slide-in-from-top-2">
                                            <div className="p-3 border-b border-gray-50 bg-gray-50/50">
                                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Select up to 5 districts</p>
                                            </div>
                                            <div className="overflow-y-auto p-2 custom-scrollbar space-y-0.5">
                                                {DISTRICT_NAMES.map(name => {
                                                    ...
                                                    const isDisabled = !isSelected && chartDistricts.length >= 5;
                                                    return (
                                                        <button
                                                            key={name}
                                                            disabled={isDisabled}
                                                            onClick={() => {
                                                                if (isSelected) {
                                                                    setChartDistricts(prev => prev.filter(d => d !== name));
                                                                } else {
                                                                    setChartDistricts(prev => [...prev, name].slice(0, 5));
                                                                }
                                                            }}
                                                        ...
                                                        >
                                                            <span className={`text-xs font-bold tracking-wider border-b-2 pb-0.5 transition-all ${isSelected ? 'text-gray-600 border-gray-400' : 'text-gray-600 border-gray-400'}`}>
                                                                {name}
                                                            </span>
                                                            {isSelected && <Check className="w-4 h-4 ml-auto text-[#f64e24]" strokeWidth={3} />}
                                                        </button>
                                                    );
                                                })}
                                            </div>
                    ...
                    <section className="bg-white border border-gray-200 rounded-lg p-6 md:p-8 relative min-h-[450px]">
                        ...
                        >
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                ...
                                >
                                    <CartesianGrid vertical={true} stroke="#E5E7EB" strokeOpacity={1} />
                                    <XAxis
                                        dataKey="year"
                                        type="number"
                                        domain={[2011, 2036]}
                                    ...
                                    <Tooltip
                                        ...
                                        content={({ active, payload, label }) => {
                                            if (active && payload && payload.length && hoveredDistrict) {
                                                const filteredPayload = payload.filter(p => (p.dataKey as string).startsWith(hoveredDistrict));
                                                    ...
                                                    <div className="bg-white border-0 shadow-xl rounded-none p-3 md:p-4 min-w-[150px]">
                                                        ...
                                                        <div className="space-y-2">
                                                            {uniqueEntries.map((p: any) => {
                                                                ...
                                                                return (
                                                                    <div key={rawDataKey} className="flex justify-between items-center text-xs gap-4">
                                                                        <span className="font-bold text-gray-700">{hoveredDistrict} ({type === 'model' ? 'Model' : 'Census'})</span>
                                                                        <span className="font-mono text-[#0868ac] font-bold">{(val / 1000000).toFixed(2)}M</span>
                                                                    </div>
                                                                );
                                                            })}
                                                        </div>
                                    ...
                                    {displayDistricts.map((district) => {
                                        ...
                                        const isSomethingSelected = selectedDistrict !== 'Odisha' && selectedDistrict !== 'All Districts' && selectedDistrict !== '';
                                                ...
                                                >
                                                    <LabelList
                                                        ...
                                                        content={(props: any) => {
                                                            ...
                                                            if (index !== PROJECTION_DATA.length - 1 || value === undefined || value === null) return null;
                                                            return (
                                                                <text
                                                                    x={x + 10}
                                                                    y={y - 5}
                        ...
                        <div className="mt-8 pt-6 border-t border-gray-100 flex flex-wrap items-center justify-center gap-8">
                            ...
                            <div className="flex items-center gap-6">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Model</span>
                                </div>
                                <div className="flex items-center gap-3">
                                    <div className="w-8 h-1 border-t-2 border-dashed border-[#F76000]"></div>
                                    <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Model Projection</span>
                                </div>
                            </div>
                            ...
                            {projectionMode === 'Model Vs Census Projection' && (
                                <div className="flex items-center gap-6">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Census Projection</span>
                                    </div>
                                </div>
                            )}
                        </div>
                ...
                {isDistrictSelected && (
                    <>
                        ...
                        <div className="w-full mb-8">
                            <MapSentinelQuaterly targetDistrict={selectedDistrict} targetBounds={mapBounds} />
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                            <div>
                                ...
                                <p className="text-sm text-gray-500 mt-1 font-medium">
                                    Visualizing spatiotemporal changes in {selectedDistrict}. Click on the markers on the map to navigate to the “What Changed?”, “How It Changed,” and “Why It Changed” sections.
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col lg:flex-row items-center justify-between gap-6 mb-4">
                            ...
                            <div className="flex items-center gap-4 shrink-0 transition-all bg-gray-50/50 p-1.5 px-3 rounded-lg border border-gray-100">
                                ...
                                <div className="relative flex items-center gap-1.5 group">
                                    ...
                                    >
                                        <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${openYear1 ? 'rotate-180' : ''}`} strokeWidth={2.5} />
                                    </button>
                                    {openYear1 && (
                                        <div className="absolute right-0 top-full mt-3 bg-white rounded-md shadow-xl border border-gray-100 p-3 z-[200] animate-in fade-in slide-in-from-top-2 min-w-[120px] transition-all">
                                            {availableYears.map(y => (
                                                ...
                                                >
                                                    <span className={`text-xs font-bold tracking-wider border-b-2 pb-0.5 transition-all ${year1 === parseInt(y) ? 'text-gray-600 border-gray-400' : 'text-gray-600 border-gray-400'}`}>
                                                        {y}
                                                    </span>
                                                </button>
                                            ))}
                                ...
                                <span className="text-gray-400 font-bold text-[10px] uppercase tracking-widest">vs</span>
                                ...
                                <div className="relative flex items-center gap-1.5 group">
                                    ...
                                    <button
                                        ...
                                        className="text-[#f64e24] transition-all hover:scale-110"
                                    >
                                        <ChevronDown className={`w-4 h-4 transition-transform duration-300 ${openYear2 ? 'rotate-180' : ''}`} strokeWidth={2.5} />
                                    </button>
                                    {openYear2 && (
                                        <div className="absolute right-0 top-full mt-3 bg-white rounded-md shadow-xl border border-gray-100 p-3 z-[200] animate-in fade-in slide-in-from-top-2 min-w-[120px] transition-all">
                                            {availableYears.map(y => (
                                                <button
                                                    ...
                                                    className="w-full px-2 py-3 text-left transition-all hover:bg-gray-50/50 group/item flex items-center rounded"
                                                >
                                                    <span className={`text-xs font-bold tracking-wider border-b-2 pb-0.5 transition-all ${year2 === parseInt(y) ? 'text-gray-600 border-gray-400' : 'text-gray-600 border-gray-400'}`}>
                                                        {y}
                                                    </span>
                                                </button>
                                            ))}
                        ...
                        <MultiMapCompare
                            ...
                            activeLayer={compareLayer === 'builtup' ? 'builtup' : 'nightlight'}
                            onDistrictSelect={onDistrictSelect}
                        />
                    </>
                )}
                ...
                {showSentinel && (
                    <div className="w-full mb-8">
                        <MapSentinel targetDistrict={selectedDistrict} targetBounds={mapBounds} />
                    </div>
                )}
                ...
                <div className="space-y-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 mt-1 font-medium">Comprehensive district-level demographic indicators including total population, density, and gender distribution for the year 2025.</p>
                        </div>
                    </div>

                    <section className="bg-white border border-gray-200 rounded-lg p-6 md:p-8 overflow-hidden">
                        <div id="regional-matrix-container" className="overflow-x-auto overflow-y-auto max-h-[650px] custom-scrollbar rounded-lg border border-gray-100 relative">
                                ...
                                <tbody className="divide-y divide-gray-50">
                                    {visibleDistricts.map((d: any, index: number) => {
                                        if (d.isSeparator) {
                                            return (
                                                <tr key={`separator-${index}`} className="bg-white/50">
                                                    <td colSpan={8} className="px-6 py-2 text-center text-xs font-medium text-gray-400 tracking-widest uppercase">
                                                        ...
                                                    </td>
                                                </tr>
                                            );
                                        ...
                                        return (
                                            ...
                                            >
                                                <td className={`px-6 py-4 text-center font-bold transition-colors first:rounded-l-xl last:rounded-r-xl ${selectedDistrict?.trim().toLowerCase() === d.name.trim().toLowerCase() ? 'text-white' : 'text-gray-400 group-hover:text-primary'}`}>#{d.rank}</td>
                                                <td className={`px-6 py-4 font-bold ${selectedDistrict?.trim().toLowerCase() === d.name.trim().toLowerCase() ? 'text-white' : 'text-gray-900'}`}>{d.name}</td>
                                                <td className={`px-6 py-4 text-right font-medium ${selectedDistrict?.trim().toLowerCase() === d.name.trim().toLowerCase() ? 'text-white' : 'text-gray-600'}`}>{d.population}</td>
                                                <td className={`px-6 py-4 text-right font-medium ${selectedDistrict?.trim().toLowerCase() === d.name.trim().toLowerCase() ? 'text-white' : 'text-gray-600'}`}>{d.density}</td>
                                                <td className={`px-6 py-4 text-right font-medium text-[#1A5BAB] ${selectedDistrict?.trim().toLowerCase() === d.name.trim().toLowerCase() ? 'text-blue-100 font-bold' : ''}`}>{d.male}</td>
                                                <td className={`px-6 py-4 text-right font-medium text-[#358221] ${selectedDistrict?.trim().toLowerCase() === d.name.trim().toLowerCase() ? 'text-green-100 font-bold' : ''}`}>{d.female}</td>
                                                <td className={`px-6 py-4 text-right first:rounded-l-xl last:rounded-r-xl`}>
                                                    <span className={`px-2 py-1 rounded ${selectedDistrict?.trim().toLowerCase() === d.name.trim().toLowerCase() ? 'bg-white/20 text-white font-bold' : 'text-green-600 bg-green-50/50 font-bold'}`}>
                                                        {d.growth}
                                                    </span>
                                                </td>
                                                        ...
                                                        <ResponsiveContainer width="100%" height="100%">
                                                            <AreaChart data={d.trendData}>
                                                                {/* <Tooltip
                                                                                    ...
                                                                                    /> */}
                                                                <defs>
                                                                    <linearGradient id={`gradient-${d.rank}`} x1="0" y1="0" x2="0" y2="1">
                                                                        <stop offset="5%" stopColor={selectedDistrict?.trim().toLowerCase() === d.name.trim().toLowerCase() ? '#fff' : '#F58220'} stopOpacity={0.5} />
                                                                        <stop offset="95%" stopColor={selectedDistrict?.trim().toLowerCase() === d.name.trim().toLowerCase() ? '#fff' : '#F58220'} stopOpacity={0} />
                                                                    </linearGradient>
                                                                </defs>
                                                                <Area
                                                                    type="monotone"
                                                                    dataKey="value"
                                                                    stroke={selectedDistrict?.trim().toLowerCase() === d.name.trim().toLowerCase() ? '#fff' : '#F58220'}
                                                                    strokeWidth={1.5}
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h3 className="text-2xl font-black text-gray-900 tracking-tight flex items-center gap-3">
                                ...
                                Development Activities                            </h3>
                            <p className="text-sm text-gray-500 mt-1 font-medium">Core developments in Anugul</p>
                        </div>
                    </div>
                    ...
                    <section className="bg-white border border-gray-200 rounded-lg p-6 md:p-8 relative">
                        ...
                        >
                            ...
                            <div className="bg-white p-4 rounded-lg border-t border-gray-200 min-w-[85vw] md:min-w-[calc(33.333%-16px)] max-w-[85vw] md:max-w-[calc(33.333%-16px)] shrink-0 snap-start">
                                <h4 className="text-xs font-bold text-[#F58220] uppercase mb-2">Coal‑ and power‑led industrialisation deepening</h4>
                                <p className="text-sm text-[#F58220] leading-relaxed">
                                    Talcher hosts India’s largest power‑grade coalfield, with an estimated 168,000 workers in Anugul dependent on coal; studies suggest production will peak within the next decade then decline after 2040.
                                </p>
                            </div>
                            <div className="bg-white p-4 rounded-lg border border-gray-200 min-w-[85vw] md:min-w-[calc(33.333%-16px)] max-w-[85vw] md:max-w-[calc(33.333%-16px)] shrink-0 snap-start">
                                <h4 className="text-xs font-bold text-[#F58220] uppercase mb-2">Major upgrades in road and rail logistics</h4>
                                <p className="text-sm text-[#F58220] leading-relaxed">
                                    The four‑laning of NH‑55 via Anugul is nearing completion stretches already operational.                                </p>
                            </div>
                            <div className="bg-white p-4 rounded-lg border border-gray-200 min-w-[85vw] md:min-w-[calc(33.333%-16px)] max-w-[85vw] md:max-w-[calc(33.333%-16px)] shrink-0 snap-start">
                                <h4 className="text-xs font-bold text-[#F58220] uppercase mb-2">Anugul railway station becoming a modern hub</h4>
                                <p className="text-sm text-[#F58220] leading-relaxed">
                                    Anugul station is being redeveloped under the Amrit Bharat / Amrit Station Scheme with about ₹25.4 crore sanctioned, and over half the physical work done by 2025.                                </p>
                            </div>
                            <div className="bg-white p-4 rounded-lg border border-gray-200 min-w-[85vw] md:min-w-[calc(33.333%-16px)] max-w-[85vw] md:max-w-[calc(33.333%-16px)] shrink-0 snap-start">
                                <h4 className="text-xs font-bold text-[#F58220] uppercase mb-2">Talcher coal rail corridors and new mines</h4>
                                <p className="text-sm text-[#F58220] leading-relaxed">
                                    The Talcher coalfield inner and outer rail corridors (₹4,882 crore total) are being taken up to raise MCL’s coal dispatch by rail to about 88% (162.8 MT) by 2029–30, with Phase‑I  already commissioned.                                </p>
                            </div>
                            <div className="bg-white p-4 rounded-lg border border-gray-200 min-w-[85vw] md:min-w-[calc(33.333%-16px)] max-w-[85vw] md:max-w-[calc(33.333%-16px)] shrink-0 snap-start">
                                <h4 className="text-xs font-bold text-[#F58220] uppercase mb-2">Mahanadi Coal Railway Ltd Phase‑II (Balram–Jarpada–Tentuloi)</h4>
                                <p className="text-sm text-[#F58220] leading-relaxed">
                                    Phase‑II of the Mahanadi Coal Railway (Balram–Jarpada–Tentuloi, about 54 km) is targeted for commissioning by December 2025 and is designed to evacuate around 58 MT of coal annually from CIL and non‑CIL blocks in the southern and central Talcher coalfield.</p>
                            </div>
                            <div className="bg-white p-4 rounded-lg border border-gray-200 min-w-[85vw] md:min-w-[calc(33.333%-16px)] max-w-[85vw] md:max-w-[calc(33.333%-16px)] shrink-0 snap-start">
                                <h4 className="text-xs font-bold text-[#F58220] uppercase mb-2">New district‑level road and education projects</h4>
                                <p className="text-sm text-[#F58220] leading-relaxed">
                                    In July 2025, the Union education minister inaugurated 34 projects worth about ₹44.9 crore and laid foundations for 19 projects worth about ₹8.9 crore in Anugul, including key road improvements (Kosala–Chhendipada, NH‑55 to Patharagada) and upgrades to educational infrastructure, which will be implemented over the next few years.</p>
                            </div>
                        </div>
                    </section>
                </div>
                ...
                <div className="space-y-6">
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            <h3 className="text-2xl font-black text-gray-900 tracking-tight flex items-center gap-3">
                                <AlertCircle className="w-6 h-6 text-black" />
                                Insights                            </h3>
                            <p className="text-sm text-gray-500 mt-1 font-medium">The demographic transformation of Anugul district from 2011 to 2024 presents a compelling narrative of industrial-driven urbanization captured through advanced geospatial analysis. By integrating satellite imagery and machine learning algorithms, this study reveals critical insights into how industrialization and urban growth has fundamentally reshaped the district's population dynamics and spatial distribution.</p>
                        </div>
                    </div>
                    ...
                    <section className="bg-white border border-gray-200 rounded-lg p-6 md:p-8 relative">
                        ...
                        >
                            ...
                            `}</style>
                            <div className="bg-white p-4 rounded-lg border-t border-gray-200 min-w-[85vw] md:min-w-[calc(33.333%-16px)] max-w-[85vw] md:max-w-[calc(33.333%-16px)] shrink-0 snap-start">
                                <h4 className="text-xs font-bold text-[#F58220] uppercase mb-2">Industrial Expansion as the Primary Growth Catalyst
                                </h4>
                                <p className="text-sm text-[#F58220] leading-relaxed">
                                    The coal mining sector has emerged as the dominant force driving Anugul's demographic evolution and has attracted substantial migration from other states and districts, creating concentrated population clusters around mining and industrial centers. The mining belt effect has fundamentally altered the district's economic structure, transforming Anugul from a predominantly agricultural region into one that continues to draw workers and their families seeking employment opportunities.
                                </p>
                            </div>
                            <div className="bg-white p-4 rounded-lg border border-gray-200 min-w-[85vw] md:min-w-[calc(33.333%-16px)] max-w-[85vw] md:max-w-[calc(33.333%-16px)] shrink-0 snap-start">
                                <h4 className="text-xs font-bold text-[#F58220] uppercase mb-2">Concentrated Urban Growth in Strategic Corridors
                                </h4>
                                <p className="text-sm text-[#F58220] leading-relaxed">
                                    Spatial analysis reveals that urbanization has occurred in a highly concentrated pattern. The Anugul-Talcher region has expanded rapidly with s and nightlight data patterns increasing between 2011 and 2024. This concentrated growth pattern indicates that economic benefits and population increases have been geographically uneven, with some areas experiencing rapid transformation while rural peripheries remain comparatively underdeveloped.                               </p>
                            </div>
                            <div className="bg-white p-4 rounded-lg border border-gray-200 min-w-[85vw] md:min-w-[calc(33.333%-16px)] max-w-[85vw] md:max-w-[calc(33.333%-16px)] shrink-0 snap-start">
                                <h4 className="text-xs font-bold text-[#F58220] uppercase mb-2">Significant Land Use Transformation and Agricultural Decline</h4>
                                <p className="text-sm text-[#F58220] leading-relaxed">
                                    The industrial expansion has necessitated substantial land use changes that have reshaped the district's physical landscape. As shown in the Agricultural land changes, increase in opencast mining activities and coalfield expansions. This dramatic shift of the district's economic base has direct implications for traditional livelihoods, food security, and the occupational profiles of the local population.</p>
                            </div>
                            <div className="bg-white p-4 rounded-lg border border-gray-200 min-w-[85vw] md:min-w-[calc(33.333%-16px)] max-w-[85vw] md:max-w-[calc(33.333%-16px)] shrink-0 snap-start">
                                <h4 className="text-xs font-bold text-[#F58220] uppercase mb-2">Population Displacement and Resettlement Dynamics
                                </h4>
                                <p className="text-sm text-[#F58220] leading-relaxed">
                                    Industrial and mining expansion has triggered significant population displacement, creating complex resettlement patterns across the district. Mining operations affect many families who then are forced to migrate and have their traditional community structures and agricultural livelihoods disrupted, requiring affected populations to adapt to new locations and often transition to non-agricultural employment in the industrial sector.                                </p>
                            </div>
                            <div className="bg-white p-4 rounded-lg border border-gray-200 min-w-[85vw] md:min-w-[calc(33.333%-16px)] max-w-[85vw] md:max-w-[calc(33.333%-16px)] shrink-0 snap-start">
                                <h4 className="text-xs font-bold text-[#F58220] uppercase mb-2">Infrastructure Development Enabling Further Growth
                                </h4>
                                <p className="text-sm text-[#F58220] leading-relaxed">
                                    The expansion of transportation networks and civic infrastructure has both facilitated and accelerated population growth in industrial centers. Improvements to National Highway 55, development of new railway lines, and the launch of multiple development projects have enhanced connectivity across the district, making it easier to transport goods and people while encouraging the establishment of new settlements and small-scale industries around major corridors.</p>
                            </div>
                                ...
                                <table className="w-full text-sm">
                                    ...
                                    <tbody className="divide-y divide-gray-50">
                                        {DISTRICT_PERFORMANCE.map((d: any) => (
                                            ...
                                            >
                                                <td className="px-6 py-4 text-center font-bold text-gray-400">#{d.rank}</td>
                                                <td className="px-6 py-4 font-bold text-gray-900">{d.name}</td>
                                                <td className="px-6 py-4 text-right font-medium text-gray-600">{d.population}</td>
                                                <td className="px-6 py-4 text-right font-medium text-gray-600">{d.density}</td>
                                                <td className="px-6 py-4 text-right font-medium text-[#1A5BAB]">{d.male}</td>
                                                <td className="px-6 py-4 text-right font-medium text-[#358221]">{d.female}</td>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="h-10 w-24 mx-auto">
                                                        <ResponsiveContainer width="100%" height="100%">
                                                            <AreaChart data={d.trendData}>
                                                                <defs>
                                                                    <linearGradient id={`gradient-modal-${d.rank}`} x1="0" y1="0" x2="0" y2="1">
                                                                        <stop offset="5%" stopColor="#F58220" stopOpacity={0.5} />
