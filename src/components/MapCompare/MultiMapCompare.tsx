...
import { Plus, X, ChevronDown, Layers, Map as MapIcon, Calendar } from 'lucide-react';
...
const LAYER_CONFIGS: any = {
    nightlight: {
        ...
        params: '#color:["#000000","#333333","#663300","#ccaa00","#ffff00"],0,200,c',
        type: 'raster'
    },
    roads: {
        label: "Road Network",
        urls: {
            "2014": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/roads/district_0_roads_2014.pmtiles",
            "2015": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/roads/district_0_roads_2015.pmtiles",
            "2016": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/roads/district_0_roads_2016.pmtiles",
            "2017": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/roads/district_0_roads_2017.pmtiles",
            "2018": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/roads/district_0_roads_2018.pmtiles",
            "2019": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/roads/district_0_roads_2019.pmtiles",
            "2020": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/roads/district_0_roads_2020.pmtiles",
            "2021": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/roads/district_0_roads_2021.pmtiles",
            "2022": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/roads/district_0_roads_2022.pmtiles",
            "2023": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/roads/district_0_roads_2023.pmtiles",
            "2024": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/roads/district_0_roads_2024.pmtiles",
            "2025": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/roads/district_0_roads_2025.pmtiles"
        },
    ...
    builtup: {
        label: "Built-up Area",
        urls: {
            "2017": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/builtup/builtup_2017.tif",
            "2018": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/builtup/builtup_2018.tif",
            "2019": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/builtup/builtup_2019.tif",
            "2020": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/builtup/builtup_2020.tif",
            "2021": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/builtup/builtup_2021.tif",
            "2022": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/builtup/builtup_2022.tif",
            "2023": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/builtup/builtup_2023.tif",
            "2024": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/builtup/builtup_2024.tif",
            "2025": "https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/builtup/builtup_2025.tif",
        },
        params: '7,7',
        type: 'raster'
    },
...
const BASE_MAP_STYLE: any = {
    ...
    layers: [
        { id: 'base-grey', type: 'raster', source: 'esri-grey', layout: { visibility: 'none' } },
        { id: 'base-satellite', type: 'raster', source: 'esri-satellite', layout: { visibility: 'none' } },
...
const sharedInitialBoundsRef: { current: maplibregl.LngLatBoundsLike | null } = { current: null };
let initialBoundsFetchStarted = false;

export const MultiMapCompare: React.FC<MultiMapCompareProps> = ({ targetBounds, activeLayer: propActiveLayer, selectedDistrict, onDistrictSelect }) => {
    const [mapConfigs, setMapConfigs] = useState<MapConfig[]>([
        { id: 'map-1', year: '2024', layer: propActiveLayer || 'builtup', basemap: 'grey' },
        { id: 'map-2', year: '2024', layer: propActiveLayer || 'builtup', basemap: 'grey' }
    ]);

    const [mapsLoadedCount, setMapsLoadedCount] = useState(0);
    const [isAddingMap, setIsAddingMap] = useState(false);
    const [pendingConfig, setPendingConfig] = useState({ layer: propActiveLayer || 'builtup', year: '2024' });

    // Update existing maps if external layer changes
    useEffect(() => {
        if (propActiveLayer) {
            setMapConfigs(prev => prev.map(m => ({ ...m, layer: propActiveLayer })));
        }
    }, [propActiveLayer]);
    ...
    useEffect(() => {
        ...
        mapInstances.current.forEach((map) => {
            if (map.isStyleLoaded()) {
                map.fitBounds(boundsToUse, { padding: 40, duration: 1200, essential: true });
            } else {
                map.once('load', () => {
    ...
    const confirmAddMap = () => {
        ...
        setMapConfigs([...mapConfigs, { ...pendingConfig, id: newId, basemap: 'grey' }]);
    ...
    const updateConfig = (id: string, updates: Partial<MapConfig>) => {
        setMapConfigs(mapConfigs.map((m: MapConfig) => m.id === id ? { ...m, ...updates } : m));
    };
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <p className="text-[13px] text-gray-500 mt-1 font-medium leading-relaxed">
                        Simultaneously visualize and compare spatio-temporal demographic shifts, land use patterns, and infrastructure development across multiple interactive map panels.
                    </p>
                </div>
                        ...
                        <MapItem
                            ...
                            onMapLoad={(map: maplibregl.Map) => {
                                ...
                                if (firstMap && firstMap !== map) {
                                    ...
                                    map.jumpTo({ center: firstMap.getCenter(), zoom: firstMap.getZoom(), bearing: firstMap.getBearing(), pitch: firstMap.getPitch() });
                                    ...
                                    map.fitBounds(sharedInitialBoundsRef.current, { padding: 40, duration: 0 });
                                }
                            }}
                        />
                    </div>
                    ...
                    <div className="min-w-[450px] max-w-[450px] h-[440px] bg-white rounded-2xl border-2 border-dashed border-orange-100 p-8 flex flex-col items-center justify-center gap-6 animate-in fade-in zoom-in duration-300">
                        ...
                        <div className="w-full space-y-4">
                            <div className="relative group">
                                <label className="text-[10px] font-black uppercase tracking-widest text-[#0868ac] mb-2 block">Choose Data Layer</label>
                                <div className="relative">
                                    <select
                                        value={pendingConfig.layer}
                                        onChange={(e) => {
                                            const layer = e.target.value;
                                            const defaultYear = mapConfigs.length === 2 ? '01-01-2026' : '2024';
                            ...
                            <div className="relative group">
                                <label className="text-[10px] font-black uppercase tracking-widest text-[#0868ac] mb-2 block">Date</label>
                                <div className="relative">
                                    <select
                                        value={pendingConfig.year}
                                        onChange={(e) => setPendingConfig({ ...pendingConfig, year: e.target.value })}
                                        className="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3.5 text-[13px] font-black text-gray-700 focus:ring-2 focus:ring-orange-500 transition-all outline-none appearance-none cursor-pointer"
                                    >
                                        {mapConfigs.length === 2 ? (
                                            MONTHLY_DATES.map(y => <option key={y} value={y}>{y}</option>)
                                        ) : (
                                            Object.keys(LAYER_CONFIGS[pendingConfig.layer]?.urls || {}).map(y => <option key={y} value={y}>{y}</option>)
                                        )}
                                    </select>
...
const MapItem = ({ config, panelIndex, selectedDistrict, onRemove, onUpdate, onMapLoad, onDistrictSelect }: {
    config: MapConfig;
    panelIndex: number;
    selectedDistrict?: string;
    onRemove: () => void;
    ...
    useEffect(() => {
        ...
        map.on('load', async () => {
            ...
            map.setLayoutProperty('base-grey', 'visibility', config.basemap === 'grey' ? 'visible' : 'none');
            map.setLayoutProperty('base-satellite', 'visibility', config.basemap === 'satellite' ? 'visible' : 'none');
            map.setLayoutProperty('base-osm', 'visibility', config.basemap === 'osm' ? 'visible' : 'none');
        ...
        if (currentLayerConfig) {
            ...
            if (isMonthlyRequest) {
                ...
                if (!availableYears.includes(config.year)) {
                    const fallbackYear = availableYears[availableYears.length - 1] || '2024';
    ...
    const refreshMapContent = async () => {
        ...
        const layersToClean = ['data-layer', 'districts-mask', 'districts-outline', 'subdistrict-outline'];
        const sourcesToClean = ['data-source', 'districts-source', 'subdistrict-source'];
    ...
    const addVectorOutlines = (map: maplibregl.Map) => {
        if (!map.getSource('districts-source')) {
            map.addSource('districts-source', { type: 'vector', url: `pmtiles://${PMTILES_URL}` });
            ...
            map.on('click', 'districts-mask', (e) => {
                if (e.features && e.features.length > 0 && onDistrictSelect) {
                    ...
                    const rawName = props.district_name || props.DIST_NAME || props.District || props.NAME || props.name || props.district;
            ...
            map.addLayer({
                ...
                source: 'districts-source',
                'source-layer': 'zcta',
                paint: { 'line-color': '#000000', 'line-width': 0.8, 'line-opacity': 0.5 }
            });
        }

        if (!map.getSource('subdistrict-source')) {
            map.addSource('subdistrict-source', { type: 'vector', url: `pmtiles://${SUBDISTRICT_URL}` });
            map.addLayer({
                id: 'subdistrict-outline',
                type: 'line',
                source: 'subdistrict-source',
                'source-layer': 'zcta',
                paint: { 'line-color': '#989898ff', 'line-width': 0.9, 'line-opacity': 0.8 }
            });
        }
    };
    ...
    const addDataOverlay = async (map: maplibregl.Map) => {
        ...
        if (MONTHLY_DATES.includes(yearKey) || currentConfig.type === 'sentinel') {
            const timeRange = SENTINEL_DATE_MAP[yearKey] || currentConfig.urls[yearKey] || Object.values(currentConfig.urls)[0] as string;
            if (!timeRange) return;
        ...
        const url = currentConfig.urls[yearKey] || Object.values(currentConfig.urls)[0] as string;
        ...
        if (currentConfig.type === 'raster') {
            ...
            try {
                ...
                if (metadata?.vector_layers?.[0]?.id) sourceLayerName = metadata.vector_layers[0].id;

                map.addSource(sourceId, { type: 'vector', url: `pmtiles://${httpUrl}` });
                ...
                if (config.layer === 'roads') {
                    ...
                    map.addLayer({
                        ...
                        source: sourceId,
                        'source-layer': sourceLayerName,
                        paint: { 'fill-color': activeColor, 'fill-opacity': 0.6, 'fill-outline-color': '#ffffff' }
                    });
                }
            } catch (e) {
                ...
                <div className="flex gap-2 pointer-events-auto">
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            <span className="text-[11px] font-black text-gray-700 tracking-tighter uppercase">{config.basemap}</span>
                            <ChevronDown className={`w-3 h-3 text-gray-400 transition-transform ${isBasemapOpen ? 'rotate-180' : ''}`} strokeWidth={3} />
                        </button>
                        {isBasemapOpen && (
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            <span className="text-[11px] text-gray-700 font-mono tracking-tighter">{config.year}</span>
                            <ChevronDown className={`w-3 h-3 text-gray-400 transition-transform ${isYearOpen ? 'rotate-180' : ''}`} strokeWidth={3} />
                        </button>
                        {isYearOpen && (() => {
                            const isMonthly = panelIndex === 2;
                            const options = isMonthly ? MONTHLY_DATES : Object.keys(LAYER_CONFIGS[config.layer]?.urls || {});
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            <span className="text-[11px] text-gray-700 tracking-tighter uppercase">{LAYER_CONFIGS[config.layer]?.label}</span>
                            <ChevronDown className={`w-3 h-3 text-gray-400 transition-transform ${isLayerOpen ? 'rotate-180' : ''}`} strokeWidth={3} />
                        </button>
                        {isLayerOpen && (
