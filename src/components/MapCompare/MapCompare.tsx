...
export const DATA_CONFIG_ANUGUL: any = {
  nightlight: {
    ...
    params:
      '#color:["#e0f7fa", "#b2ebf2", "#80deea", "#4dd0e1", "#26c6da"],0,20,c', // Light
  },
  urbansprawl: {
    urls: {
      '2011':
    ...
    params:
      '#color:["#0868ac","#0868ac","#0868ac","#0868ac","#0868ac"],0,3000,c', // Pop
  },
  roads: {
    urls: {
      '2015':
    ...
    if (!protocolsAdded) {
      try {
        if (!maplibregl.addProtocol.toString().includes('cog')) {
          // Check if not added or handle via try-catch
  ...
  const updateMainLayer = async (
    ...
    if (activeLayerKey === 'sentinel2') {
      ...
      try {
        ...
        if (map.getSource(sourceId)) {
          // Check if source still exists (async safety)
            ...
            <div className="absolute top-0 bottom-0 right-0 w-70 bg-gradient-to-l from-black/70 via-black/30 to-transparent flex flex-col pt-20 pb-4 px-6 z-30 pointer-events-none items-end text-right">
              <div className="text-white mt-4 flex flex-col items-end w-full">
                ...
                <div className="flex justify-end">
                  <span
                    className={`flex items-center gap-1.5 px-2 py-1 rounded-sm text-[10px] font-black tracking-wide shadow-sm ${isPos ? 'bg-white/20 text-[#a7f3d0] border border-[#a7f3d0]/30' : 'bg-white/20 text-red-300 border border-red-300/30'}`}
                  >
                    {isPos ? (
