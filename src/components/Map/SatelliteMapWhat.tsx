...
const PMTILES_URL_CHANGE = 'https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/Data/anugul_change_analysis.pmtiles';
...
export const SatelliteMapWhat: React.FC<SatelliteMapProps> = ({
    ...
    const [imageModal, setImageModal] = useState<{ id: number; title: string; desc: string } | null>(null);

    // Bind window method for popup button clicks
    useEffect(() => {
        (window as any).showMapImage = (id: number) => {
            const item = tabContentRef.current?.find(c => String(c.id) === String(id));
            setImageModal({
                id,
                title: item?.title || `Analysis ${id}`,
                desc: item?.desc || 'Satellite imagery analysis displaying significant structural changes.'
            });
        };
        return () => {
                ...
                map.once('idle', () => {
                    ...
                    Object.values(unique).forEach((f: any) => {
                        ...
                        if (!bounds.isEmpty()) {
                            ...
                            const item = tabContentRef.current?.find(c => String(c.id) === String(props.id));
                            const popupTitle = item?.title || props.title || `Analysis ${props.id}`;
                ...
                <div className="fixed inset-0 z-[2000] flex items-center justify-center p-4 md:p-8 bg-black/80 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-[#111] rounded-2xl w-full max-w-6xl max-h-[90vh] flex flex-col md:flex-row shadow-2xl relative overflow-hidden border border-white/10 animate-in zoom-in-95 duration-300">
                        ...
                        <div className="w-full md:w-1/3 p-6 md:p-8 text-white flex flex-col overflow-y-auto">
                            <h3 className="text-xl md:text-2xl font-black text-white mb-3 tracking-tight leading-tight">{imageModal.title}</h3>
