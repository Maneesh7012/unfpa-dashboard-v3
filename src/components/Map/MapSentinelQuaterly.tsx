...
import { Plus, Minus, Home, Satellite, ChevronDown, ChevronRight, ChevronLeft, Calendar, X, LineChart as LucideLineChart } from 'lucide-react';
...
const LULC_LEGEND: LulcLegendItem[] = [
    ...
    { label: 'Flooded vegetation', color: '#7A87C6', value: 3, key: 'flooded_vegetation' },
    { label: 'Crops', color: '#E49635', value: 4, key: 'crops' },
    { label: 'Shrub & Scrub', color: '#DFC35A', value: 5, key: 'shrub_and_scrub' },
...
const QUARTERS = generateQuarters(2018, 2025);
const PMTILES_URL = 'https://dicratiler.blob.core.windows.net/dicra-dev/unfpa/data_v3/lulc_quarterly/od_district_lulc_quarterly.pmtiles';
...
export const MapSentinelQuaterly: React.FC<MapSentinelQuaterlyProps> = ({
    ...
    const [selectedLulcCategory, setSelectedLulcCategory] = useState<LulcCategory | null>(null);
    ...
    const [districtLulcData, setDistrictLulcData] = useState<Record<string, any> | null>(null);

    const currentQuarter = QUARTERS[selectedIdx];

    const getOrCreateMosaicUrl = useCallback(async (q: Quarter, bbox: number[]): Promise<string | null> => {
    ...
    useEffect(() => {
        ...
        if (targetBounds) {
            ...
            map.fitBounds(pmtilesBounds || ODISHA_BOUNDS, { padding: fitPadding, duration: 1500 });
    ...
    useEffect(() => {
        ...
        const normalizeName = (s: string) => s.toLowerCase().trim().replace(/[^a-z0-9]/g, '');
        ...
        const findFeature = (name: string) => {
            ...
            for (const [canonical, synonyms] of Object.entries(DISTRICT_SYNONYMS)) {
                if (normalizeName(canonical) === normalizedTarget || synonyms.some(s => normalizeName(s) === normalizedTarget)) {
                    // Search for canonical or any synonym in the cache
                    for (const term of [canonical, ...synonyms]) {
                        const features = accumulatedFeaturesRef.current.get(term.toLowerCase());
        ...
        if (isOdisha) {
            ...
            accumulatedFeaturesRef.current.forEach((features) => {
                features.forEach((f) => {
                    const rawName = f.properties?.district_name || f.properties?.NAME || f.properties?.name || f.properties?.District_N;
                    ...
                    if (id && !uniqueFeatureIds.has(id)) {
                        ...
                        Object.keys(props).forEach(key => {
                            if (key.startsWith('lulc_')) {
                                aggregated[key] = (aggregated[key] || 0) + (Number(props[key]) || 0);
                            }
                        });
                    }
                });
            });
            setDistrictLulcData(Object.keys(aggregated).length > 0 ? aggregated : null);
        } else {
    ...
    useEffect(() => {
        ...
        const preRegisterAll = async () => {
            ...
            for (const chunk of chunks) {
                await Promise.all(chunk.map(q => getOrCreateMosaicUrl(q, ODISHA_BBOX)));
            }
        };
        preRegisterAll();
    ...
    useEffect(() => {
        ...
        setColorFunction(baseLulcUrl, (pixel: any, color: any, metadata: any) => {
            const val = pixel[0];
            if (val === metadata.noData || val < 0 || val > 7) { color.set([0, 0, 0, 0]); return; }
            const rgba = [...(LULC_RGBA[val] || [0, 0, 0, 0])];
            let isVisible = selectedLulcCategory === 'all';
            if (typeof selectedLulcCategory === 'number') isVisible = val === selectedLulcCategory;
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-[13px] text-gray-500 mt-1 font-medium ">
                        Analyze temporal shifts in landscape categories.</p>
                </div>
                <div className="flex items-center gap-3 bg-gray-50/50 p-1.5 px-3 rounded-lg border border-gray-100 relative">
                    ...
                    >
                        <span className="flex items-center gap-2">
                            <span className="w-2 h-2 rounded-full" style={{ backgroundColor: '#F76000' }} />
                        ...
                        <>
                            ...
                            <div className="absolute right-0 top-full mt-3 bg-white rounded-xl shadow-2xl border border-gray-100 p-3 z-[200] min-w-[180px] max-h-[400px] overflow-y-auto custom-scrollbar">
                                {QUARTERS.slice().reverse().map((q, idx) => {
                                    ...
                                    return (
                                        <button key={q.key} onClick={() => { setSelectedIdx(originalIdx); setIsDropdownOpen(false); }} className={`w-full px-3 py-2 text-left hover:bg-gray-50 flex items-center justify-between rounded-lg ${selectedIdx === originalIdx ? 'bg-orange-50 text-[#F76000]' : 'text-gray-600'}`}>
                                            <span className="flex items-center gap-2">
                                                <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: '#F76000' }} />
            ...
            <section className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-xl h-[700px] relative">
                ...
                {isLoaded && (
                    <div className={`absolute top-8 left-8 w-[300px] h-fit transition-all duration-300 text-gray-900 bg-white/95 backdrop-blur-md rounded-2xl border border-gray-200 p-6 shadow-2xl z-[60] flex flex-col gap-6 overflow-y-auto custom-scrollbar`}
                        ...
                        <div className="flex-1 transition-all">
                            ...
                            <div className="flex flex-col gap-1.5">
                                <button onClick={() => setSelectedLulcCategory(prev => prev === 'all' ? null : 'all')} className={`flex items-center justify-between px-4 py-3 rounded-xl border transition-all ${selectedLulcCategory === 'all' ? 'bg-orange-50 border-[#F76000] shadow-sm' : 'bg-white border-gray-100 text-gray-500 hover:bg-gray-50'}`}>
                                    <div className="flex items-center gap-3">
                                        <div className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center transition-all ${selectedLulcCategory === 'all' ? 'border-[#F76000]' : 'border-gray-200'}`}>
                                            {selectedLulcCategory === 'all' && <div className="w-1.5 h-1.5 rounded-full bg-[#F76000]" />}
                                        </div>
                                        <span className={`text-[10px] font-black uppercase ${selectedLulcCategory === 'all' ? 'text-[#F76000]' : 'text-gray-600'}`}>All Classes</span>
                                    </div>
                                </button>
                                {LULC_LEGEND.map(item => (
                                    <button key={item.label} onClick={() => setSelectedLulcCategory(prev => prev === item.value ? null : item.value)} className={`flex items-center justify-between px-4 py-3 rounded-xl border transition-all ${selectedLulcCategory === item.value ? 'bg-orange-50 border-[#F76000] shadow-sm' : 'bg-white border-gray-100 text-gray-500 hover:bg-gray-50'}`}>
                                        <div className="flex items-center gap-3">
                                            <div className={`w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center transition-all ${selectedLulcCategory === item.value ? 'border-[#F76000]' : 'border-gray-200'}`}>
                                                {selectedLulcCategory === item.value && <div className="w-1.5 h-1.5 rounded-full bg-[#F76000]" />}
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <div className="w-2.5 h-2.5 rounded-sm ring-1 ring-gray-100" style={{ backgroundColor: item.color }} />
                                                <span className={`text-[10px] font-bold capitalize ${selectedLulcCategory === item.value ? 'text-[#F76000]' : 'text-gray-600'}`}>{item.label}</span>
                                            </div>
                                        </div>
                                    </button>
                                ))}
                ...
                {isLoaded && (
                    ...
                    >
                        {/* Timeline Slider */}
                        <div ref={timelineCardRef} className="flex-1 bg-white/95 backdrop-blur-md rounded-2xl border border-gray-200 p-5 px-6 shadow-2xl flex items-center gap-6">
                            ...
                            <div className="flex-1 flex flex-col gap-4">
                                <div className="flex justify-between items-end text-[8px] font-black uppercase tracking-[0.2em]">
                                    ...
                                    <div className="text-right flex items-center gap-2">
                                        <span className="text-[#F76000] text-lg font-mono font-bold leading-none">{currentQuarter.label.split(' ')[0]}</span>
                                        ...
                                        <ResponsiveContainer width="100%" height="100%">
                                            <LineChart data={chartData} margin={{ top: 5, right: 5, left: 5, bottom: 0 }}>
                                                        ...
                                                        if (active && payload && payload.length) {
                                                            return (
                                                                <div className="bg-white/95 backdrop-blur-md border border-gray-200 p-2.5 rounded-lg shadow-xl text-[10px]">
                                                                    <p className="font-black text-gray-900 mb-1 border-b pb-1">{label}</p>
                                                                    {payload.map((p: any) => (
                                                                        <div key={p.dataKey} className="flex items-center justify-between gap-4 py-0.5">
                                                                            <div className="flex items-center gap-1.5">
                                                                                <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: p.color }} />
                                                                                <span className="font-bold text-gray-600 capitalize">{p.name.replace('_', ' ')}</span>
                                                                            </div>
                                                                            <span className="font-mono font-black text-[#F76000]">{Number(p.value).toFixed(1)} <span className="text-[8px] opacity-60">sqkm</span></span>
                                                                        </div>
                                                                    ))}
                                                                </div>
                                                            );
                                                ...
                                                {selectedLulcCategory === 'all' ? (
                                                    ...
                                                    <Line
                                                        type="monotone"
                                                        dataKey={LULC_LEGEND.find(c => c.value === selectedLulcCategory)?.key || ''}
                                                        stroke={LULC_LEGEND.find(c => c.value === selectedLulcCategory)?.color || '#F76000'}
                                                        strokeWidth={3}
                                                        dot={(props: any) => {
                                                            const { cx, cy, index } = props;
                                                            if (index === selectedIdx) {
                                                                return <Dot cx={cx} cy={cy} r={4} fill="#F76000" stroke="#FFFFFF" strokeWidth={2} />;
                                                            }
                                                            return null;
                                                        }}
                                                        isAnimationActive={false}
                                                ...
                                                {/* Active Marker */}
                                                <ReferenceLine x={currentQuarter.label} stroke="#F76000" strokeWidth={2} strokeDasharray="3 3" />
                                            </LineChart>
                                        </ResponsiveContainer>
                                    </div>
                                )}
                                ...
                                <div className="relative group/track py-1">
                                    ...
                                    <div className="relative h-2 bg-gray-100 rounded-full overflow-visible">
                                        <div className="absolute h-full rounded-full transition-all duration-300" style={{ width: `${(selectedIdx / (QUARTERS.length - 1)) * 100}%`, backgroundColor: '#F76000' }} />
                                        ...
                                        <div className="absolute inset-0 flex justify-between items-center px-0.5 pointer-events-none">
                                            {QUARTERS.map((q, idx) => (
                                                <div key={q.key} className="relative flex flex-col items-center">
                                                    <div className={`w-[2px] h-3 rounded-full mb-1 transition-all ${idx === selectedIdx ? 'bg-[#F76000] h-4' : 'bg-gray-300'}`} />
                                                    ...
                                                    <div className="absolute -bottom-5">
                                                        <span className={`text-[8px] font-bold transition-all ${idx === selectedIdx ? 'text-[#F76000] scale-110' : 'text-gray-400 opacity-60'}`}>
                                                            {q.label.charAt(0)}
                                                        </span>
                                                    </div>
                                                </div>
                            ...
                            <div className="flex flex-col gap-2 items-center">
                                <button
                                    ...
                                    title={!showLulc ? "Select a category first" : (showChart ? "Hide Chart" : "Show Chart")}
                                >
                                    {showChart ? <X className="w-5 h-5" /> : <LucideLineChart className="w-5 h-5" />}
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-2 shrink-0 self-end pb-1">
                            <button onClick={handleReset} className="bg-white/90 backdrop-blur-md w-9 h-9 flex items-center justify-center rounded-xl shadow-lg border border-gray-100 text-gray-600 hover:text-[#F76000] hover:border-[#F76000] transition-all active:scale-90" title="Reset View">
                                <Home className="w-4 h-4" />
                            </button>
                            <button onClick={() => mapRef.current?.zoomIn()} className="bg-white/90 backdrop-blur-md w-9 h-9 flex items-center justify-center rounded-xl shadow-lg border border-gray-100 text-gray-600 hover:text-[#F76000] hover:border-[#F76000] transition-all active:scale-90" title="Zoom In">
                                <Plus className="w-4 h-4" />
                            </button>
                            <button onClick={() => mapRef.current?.zoomOut()} className="bg-white/90 backdrop-blur-md w-9 h-9 flex items-center justify-center rounded-xl shadow-lg border border-gray-100 text-gray-600 hover:text-[#F76000] hover:border-[#F76000] transition-all active:scale-90" title="Zoom Out">
                                ...
                                return (
                                    <div className="flex flex-col w-full text-gray-800">
                                        ...
                                        <div className="flex items-center gap-2 text-[11px] text-gray-500 mb-2 font-mono">
                                            <Calendar className="w-3.5 h-3.5 text-[#F76000]" strokeWidth={2.5} />
                                        ...
                                        {currentContent.content ? (
                                            currentContent.content.map((block: any, idx: number) => {
                                                if (block.type === 'heading') {
                                                    return (
                                                        <h4 key={idx} className="text-sm font-black text-gray-900 mb-3 mt-2 border-l-4 border-[#F76000] pl-2">
                                                            {block.value}
                                                        </h4>
                                                    );
