...
import { ChevronDown, Play, Pause, RotateCcw, Plus, Minus, Home, Satellite } from 'lucide-react';
...
export const MapSentinel: React.FC<MapSentinelProps> = ({ targetDistrict = 'Odisha', targetBounds }) => {
    ...
    const switchLayer = useCallback((year: string) => {
        ...
        YEARS.forEach(y => {
            if (map.getLayer(`s2-${y}`)) {
                map.setLayoutProperty(`s2-${y}`, 'visibility', y === year ? 'visible' : 'none');
            }
        });
    }, []);
    ...
    useEffect(() => {
        ...
        if (targetBounds) {
            map.fitBounds(targetBounds, { padding: 80, duration: 1500, essential: true });
            return;
        }
    ...
    useEffect(() => {
        ...
        const map = new maplibregl.Map({
            ...
            minZoom: 5,
            maxZoom: 14,
            maxBounds: [[79.277344, 16.232218], [90.000000, 24.058806]],
        ...
        map.on('sourcedata', (e) => {
            if (e.sourceId === 'district-source-sentinel' && e.isSourceLoaded) {
                const features = map.querySourceFeatures('district-source-sentinel', { sourceLayer: 'zcta' });
                            ...
                            <>
                                ...
                                <div className="absolute right-0 top-full mt-3 bg-white rounded-md shadow-xl border border-gray-100 p-3 z-[200] animate-in fade-in slide-in-from-top-2 min-w-[120px] transition-all">
                                    {YEARS.map(y => (
                                        ...
                                        >
                                            <span className={`text-xs font-bold tracking-wider border-b-2 pb-0.5 transition-all ${selectedYear === y ? 'text-gray-600 border-gray-400' : 'text-gray-600 border-gray-400'}`}>
                                                {y}
                                            </span>
                                        </button>
                                    ))}
                    ...
                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 w-[90%] md:w-[600px] bg-gray-900/80 backdrop-blur-md rounded-2xl border border-white/20 p-4 shadow-2xl flex items-center gap-6 group transition-all hover:bg-gray-900">
                        ...
                        <div className="flex-1 flex flex-col gap-2">
                            <div className="flex justify-between items-end mb-1">
                                <span className="text-[10px] font-black text-white/40 tracking-widest uppercase">Select Year</span>
