...
// NEW: category -> marker fill colour. Lets the map render Mines vs Industry
// vs Urban Sprawl distinctly. Falls back to the original orange for anything
// unknown so the existing default style still applies.
...
export const WhatHowWhy_v2: React.FC<WhatHowWhy_v2Props> = ({
   ...
   *  Now uses the `district` field on Points_Data as a fast-path; falls
   *  back to point-in-polygon when district names don't line up. */
  const filteredPoints = React.useMemo(() => {
    ...
    // Fast-path: data already carries the district name (and aliases like
    // "Anugul" -> "Angul", "Keonjhar" -> "Kendujhar" still resolve).
    ...
    // Fallback: polygon containment (legacy path, still works for any
    // dataset that doesn't expose the district field).
  ...
  useEffect(() => {
    ...
    const geojson = {
      ...
      features: filteredPoints.map((item: any) => ({
        ...
        properties: {
          id: item.id,
          // NEW: surface category so the marker can colour itself.
  ...
  useEffect(() => {
    ...
    map.on('load', async () => {
      ...
      const showPointTooltip = (e: any) => {
        ...
        // NEW: also surface settlement_type + new built-up area + confidence
        // in the hover popup so users get a one-glance signal.
