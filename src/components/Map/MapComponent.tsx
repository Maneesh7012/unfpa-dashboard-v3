...
import { DISTRICT_NAME_VARIANTS, ALLOWED_DISTRICTS, GENDER } from '../../data/comparativeData';
...
export const MapComponent: React.FC<MapComponentProps> = ({
    ...
    useEffect(() => { showSubdistrictRef.current = showSubdistrict; }, [showSubdistrict]);
    ...
    useEffect(() => {
        ...
        if (!targetDistrict || targetDistrict === 'Odisha' || targetDistrict === 'All Districts') {
        ...
        for (const feature of accumulatedFeaturesRef.current.values()) {
            ...
            const canonicalName = DISTRICT_NAME_VARIANTS[normalizedRaw] || normalizedRaw;

            if (canonicalName?.toLowerCase() === targetDistrict.trim().toLowerCase()) {
        ...
        if (targetFeature?.geometry) {
            ...
            if (!bounds.isEmpty()) {
                ...
                onDistrictClick?.({ ...targetFeature.properties, bounds: bounds.toArray() });
            }
        }
    }, [targetDistrict]);
    ...
    const [activeBasemap, setActiveBasemap] = useState<BasemapType>(initialBasemap);
        ...
        if (!map.getLayer('subdistricts-fill')) {
            ...
            map.on('mousemove', 'subdistricts-fill', (e) => {
                if (e.features && e.features.length > 0) {
                    ...
                    const canonicalName = DISTRICT_NAME_VARIANTS[rawDistName] || rawDistName;
                    ...
                    const canonicalDistName = DISTRICT_NAME_VARIANTS[distName] || distName;

                    let rawVal = props[propKey];
                    // Fallback to static GENDER data if PMTiles property is missing and a specific gender is selected
                    if ((rawVal === undefined || rawVal === null) && currentGender !== 'All' && GENDER[canonicalDistName]) {
                        const genderKey = currentGender === 'Male' ? `${currentYear}_male` : `${currentYear}_female`;
                    ...
                    const valDisplay = (rawVal !== undefined && rawVal !== null) ? valFormat(rawVal) : '—';
                    ...
                    if (currentLayer === 'deg_urbanisation' && currentGender === 'All') {
                        const cityVal = props[`deg_urban_city_${currentYear}`] || props[`deg_city_${currentYear}`] || 0;
                        const townVal = props[`deg_urban_town_${currentYear}`] || props[`deg_town_${currentYear}`] || 0;
                        const ruralVal = props[`deg_urban_rural_${currentYear}`] || props[`deg_rural_${currentYear}`] || 0;
            ...
            map.on('click', 'subdistricts-fill', (e) => {
                if (e.features && e.features.length > 0) {
                    const feature = e.features[0];
                    const rawDistName = feature.properties?.district_name || feature.properties?.DIST_NAME;

                    if (rawDistName) {
                        const canonicalName = DISTRICT_NAME_VARIANTS[rawDistName] || rawDistName;
                        if (!ALLOWED_DISTRICTS.includes(canonicalName)) return;
                        // Find the corresponding district feature in our accumulated features
                        const distFeature = accumulatedFeaturesRef.current.get(canonicalName);
        ...
        map.on('load', async () => {
            ...
            try {
                ...
                if (header.minLon !== undefined) {
                    ...
                    map.fitBounds(bounds, { padding: { right: 0, left: 80, top: 80, bottom: 80 }, duration: 0 });
                }
            } catch (e) {
                ...
                if (source && layer) {
                    ...
                    if (features && features.length > 0) {
                        ...
                        features.forEach((f: any) => {
                            const name = f.properties?.district_name || f.properties?.NAME || f.properties?.name;
                        ...
                        if (hasChanges && onDataLoadRef.current) {
                            ...
                            uniqueFeaturesArray.sort((a: any, b: any) => {
                                const nameA = a.properties?.district_name || a.properties?.NAME || a.properties?.name || '';
                                const nameB = b.properties?.district_name || b.properties?.NAME || b.properties?.name || '';
                                return nameA.localeCompare(nameB);
                            });

                            const propertiesArray = uniqueFeaturesArray.map(f => f.properties);
                ...
                if (subSource && subLayer && showSubdistrict) {
                    ...
                    if (subFeatures && subFeatures.length > 0) {
                        ...
                        subFeatures.forEach((f: any) => {
                            const name = f.properties?.subdistrict_name || f.properties?.SUBDIST_NAM || f.properties?.fid;
    ...
    useEffect(() => {
        ...
        const handleStyleLoad = () => {
            if (map.isStyleLoaded()) {
                ...
                const rasterLayers = ['esri-satellite-layer', 'carto-dark-layer', 'esri-grey-layer'];
    ...
    useEffect(() => {
        const updateColors = () => {
            ...
            const nameExpr = ['coalesce', ['get', 'district_name'], ['get', 'DIST_NAME'], ['get', 'NAME'], ['get', 'name']];
            const genderKey = gender === 'Male' ? `${selectedYear}_male` : `${selectedYear}_female`;
            ...
            // 1. Districts Layer
            const districtColorExp = activeLayer === 'deg_urbanisation'
                ? '#D3D3D3'
                : [
                    'interpolate', ['linear'],
                    ['coalesce', ['get', propName], valueMatch],
                    scaleValues[0], '#f0f9e8',
                    scaleValues[1], '#bae4bc',
                    scaleValues[2], '#7bccc4',
                    scaleValues[3], '#43a2ca',
                    scaleValues[4], '#0868ac'
                ];

            map.setPaintProperty('districts-fill', 'fill-color', districtColorExp as any);

            // 2. Subdistrict Layer
            if (map.getLayer('subdistricts-fill')) {
                const sScale = getSubScale(effectiveLayer);
                const subColorExp = activeLayer === 'deg_urbanisation'
                    ? '#E5E7EB'
                    : [
                        'interpolate', ['linear'],
                ...
                map.setPaintProperty('subdistricts-fill', 'fill-color', subColorExp as any);
                map.setPaintProperty('subdistricts-fill', 'fill-opacity', showSubdistrict ? 0.9 : 0);
            }
    ...
    }, [selectedYear, gender, activeLayer, showSubdistrict, subdistrictDataLoaded, onLegendDataUpdate, activeBasemap, styleLoadedCount]);
    ...
    useEffect(() => {
        ...
        if (map.getLayer('districts-fill')) {
            ...
            map.setPaintProperty('districts-fill', 'fill-opacity', showSubdistrict ? 0 : 0.9);
        }
        if (map.getLayer('districts-border')) {
            map.setLayoutProperty('districts-border', 'visibility', 'visible');
            map.setPaintProperty('districts-border', 'line-color', showSubdistrict ? '#4A5568' : '#FFFFFF');
            map.setPaintProperty('districts-border', 'line-width', showSubdistrict ? 1.5 : 0.8);
            map.setPaintProperty('districts-border', 'line-opacity', showSubdistrict ? 1 : 0.8);
            if (showSubdistrict) {
                map.moveLayer('districts-border');
            }
        }
        if (map.getLayer('subdistricts-fill')) {
            map.setLayoutProperty('subdistricts-fill', 'visibility', showSubdistrict ? 'visible' : 'none');
        }
        if (map.getLayer('subdistricts-border')) {
            map.setLayoutProperty('subdistricts-border', 'visibility', showSubdistrict ? 'visible' : 'none');
        }
        if (map.getLayer('selected-district-outline')) {
            map.moveLayer('selected-district-outline');
        }
